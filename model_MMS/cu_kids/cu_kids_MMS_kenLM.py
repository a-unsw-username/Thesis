...
os.environ["TRANSFORMERS_CACHE"] = "/srv/scratch/z5363024/hf_cache"
os.environ["HF_HOME"] = "/srv/scratch/z5363024/hf_home"
...
# directory with train valid and test /srv/scratch/speechdata/speech-corpora/children/TD/cu_dataset
...
testing_id = "finetune_CU_20250703"
...
baseline_adjusted_results_fp = baseline_results_dir + testing_id + '_baseline_adjusted_result.json'
print("--> baseline_adjusted_results_fp:", baseline_adjusted_results_fp)
...
finetuned_adjusted_results_fp = finetuned_results_dir + testing_id + '_finetuned_adjusted_result.json'
print("--> finetuned_adjusted_results_fp:", finetuned_adjusted_results_fp)
...
checkpoint = "/srv/scratch/z5363024/thesis/HuBERT_Large/model/CU/finetune_CU_20250507_lr5e-5/checkpoint-70000/"
base_model = "facebook/mms-1b"
base_model_pretrained = "facebook/mms-1b-all"
target_lang = "eng"
...
#try with subset of dataset, try with different dataset
...
set_eval_accumulation_steps = 1
print("eval_accumulation_steps:", set_eval_accumulation_steps)
...
if eval_base:
    processor = AutoProcessor.from_pretrained("facebook/mms-1b-all", target_lang=target_lang)
else:
...
# everything above max_input_length seconds is filtered out to minimise memory usage
...
#-----------------------
vocab_dict = full_vocab[target_lang]
...
training_args = TrainingArguments(
    ...
    eval_accumulation_steps=set_eval_accumulation_steps,
...
if eval_base:
    ...
    results_df.to_json(baseline_results_fp)
...
#    print("Test WER of baseline model after LLM: {:.3f}".format(wer_metric.compute(predictions=results_df["corrected_pred_str"], references=results_df["text"])))
